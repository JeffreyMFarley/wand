...
file for it. A separate agent has already confirmed this file qualifies. It
qualifies for one of two reasons: **either** at least one class, function, or
method makes a direct external call, **or** it contains a god function — a single
function or method over 400 lines — which qualifies on length alone, regardless
of whether you can find an external call in it. Your job is to produce the test.
Follow every rule in this prompt exactly. Do not mock external dependencies —
...
of a test file. **Exception:** if the file contains a god function (over 400
lines), do not bail — the god function is itself the test subject even when you
cannot find an external call in it. Go to Step 3 and write its pin test.
...
on the final return value. This is a deliberately vague *characterization pin*, not
a correctness check: it will pass even when an interior phase is broken, and that is
the point — it keeps passing while the function is refactored, so it survives the
breakup instead of blocking it.

Write this test even if the god function makes no external call you can identify —
length alone qualifies it. Its real payoff is the fixtures captured downstream from
whatever calls do run inside it: those fixtures are what make it safe to later split
the function into modular pieces. A god function with genuinely no external calls
still gets this pin test (a refactoring safety net), it simply captures no fixtures.
Do not assert on interior state, and do not try to test sub-phases separately — one
end-to-end test on the return value is the whole deliverable for a god function.
